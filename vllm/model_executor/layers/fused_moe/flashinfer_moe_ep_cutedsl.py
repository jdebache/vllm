...
class FlashInferMoeEpCutedsl:
    ...
    def _tensors(
        ...
        assert self._fc1_alpha is not None
        assert self._fc2_alpha is not None
        assert self._fc1_norm_const is not None
    ...
    def __call__(
        ...
        return self._mega_layer(
            self._tensors(hidden_states, topk_ids, topk_weights)
        )
    ...
    def warmup(self) -> None:
        ...
        topk_ids = torch.arange(
            ...
            dtype=torch.int32,
            device=device,
        ).mul_(self._num_experts // self._top_k).view(1, self._top_k)
...
_FLASHINFER_MOE_EP_CUTEDSL: list[
    weakref.ReferenceType[FlashInferMoeEpCutedsl]
] = []
